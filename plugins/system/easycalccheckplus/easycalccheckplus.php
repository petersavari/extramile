...
class PlgSystemEasyCalcCheckPlus extends JPlugin
{
    ...
    function onAfterRoute()
    {
        ...
        if ($option == 'com_kunena' && $this->params->get('kunena')) {
            if ($this->user->guest) {
                ...
                if ($document->getType() == 'html') {
                    $document->addCustomTag('<style type="text/css">a#btn_qreply, a.Kreplyclick{display:none;}</style>');
                }
            }
        }
    }
    ...
    public function onAfterRender()
    {
        ...
        if ($this->loadEcc == true) {
            ...
            // Get IP address
            $this->setSession('ip', $this->getIpAddress());
    ...
    private function customCall()
    {
        ...
        if (preg_match("@(<form[^>]*>)(.*)({easycalccheckplus})(.*</form>)@Us", $body, $matches)) {
            ...
            if (!empty($matches)) {
                ...
                $keysExclude = array('option', 'view', 'layout', 'id', 'Itemid', 'task', 'controller', 'func');
                $customCallRequestVariables = array_diff($matches_request_variables[1], $keysExclude);
                $this->setSession('check_custom_call', $customCallRequestVariables);
            }
        }
    }
