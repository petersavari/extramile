		...
		<?php foreach( $list as $i => $row ): ?>
			<div class="bt-row <?php if($i==0) echo 'bt-row-first'; else if($i==count($list)-1) echo 'bt-row-last' ?>"  style="width:<?php echo $itemWidth;?>%" >
				...
				<?php if($show_category_name_as_link) : ?>
					...
					<?php endif; ?>
					<?php if( $row->params->get('spfeatured_image')&& $align_image == "center" ): ?>
					<div class="bt-center">
					<a target="<?php echo $openTarget; ?>"
						...
						<img <?php echo $imgClass ?> src="<?php echo $row->params->get('spfeatured_image'); ?>" alt="<?php echo $row->title?>"  style="width:<?php echo $thumbWidth ;?>px;" title="<?php echo $row->title?>" />
					</a>
					</div>
					<?php endif ; ?>
